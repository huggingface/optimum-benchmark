...
def target(
    ...
    try:
        ...
        if len(str(outputs)) <= 1_000_000:
            logger.info(f"\t+ Report dictionary is small enough ({len(str(outputs))} bytes), sending it directly")
            child_connection.send(outputs)
        else:
            logger.warning(
                f"\t+ Report dictionary is too large ({len(str(outputs))} bytes), "
                "sending it through file-based communication protocol"
            )
