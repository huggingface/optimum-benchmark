...
def text_classification_preprocessing(
    ...
    def tokenize_function(examples):
        return tokenizer(
            ...
            padding=padding,
            truncation=config.truncation,
            max_length=tokenizer.model_max_length -1  if tokenizer.model_max_length != None else config.backend.model.max_position_embeddings -1 if config.backend.model.max_position_embeddings != None else 500,
        )
    print(config.backend.model.max_position_embeddings)
    dataset = dataset.map(
        tokenize_function,
        batched=True,
        remove_columns=dataset.features,
